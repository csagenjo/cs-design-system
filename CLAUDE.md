...
| DialogSimple | v1 | header Default fijo (44px) · `variant` default (280px, borde+sombra propia 12/24) / expanded (480px, min-height 680px) · botones sm/lg según variant |
...
- **`DialogSimple.jsx` — borde 0.5px de la variante `default` sin Variable de color en Figma (24/08/2026).** El borde de la tarjeta compacta (280px) es un literal (`border-black`) no ligado a ninguna Variable — a diferencia del resto de bordes del sistema. Mapeado a `--ds-borderColor-subtle` (mismo hairline que Divider/Cell) en vez de hardcodear hex, pero sin confirmar contra Figma si el literal exacto coincide. Pendiente verificación puntual, no bloqueante (elemento cosmético de bajo impacto).