...
- **Cell family · piezas pendientes (22/07/2026).** Construidos: `CellData` ✅. Pendientes: `CellHeader` (añade `sortState` none/asc/desc → `Divider` + icono de orden vía prop booleana `Order`, siempre a la derecha), `CellMore` (átomo, disparador overflow "More ›") y `CellActions` (**organismo**, máx 2 acciones Button/Link/CTALink — NO es Column More). Nodos localizados: Header `3291:24405` · Cell `3291:24354` · More `3291:24336`.
- **Cell family · Column More "Basic + con texto" (22/07/2026).** Cuando exista esa variante necesitará `paddingTop/basic`+`paddingBottom/basic` como Header. Aún NO diseñada en Figma — no inventarla.
- **Headline · lineHeight derivado (22/07/2026).** El emparejamiento tamaño↔lineHeight NO está en los JSON (vive en text styles de Figma, no en variables). Derivado por regla "menor `fontLheight` ≥ `fontSize`" con ancla confirmada `headline/md 28 ↔ fontLheight/sm 28.5`. Verificar contra los text styles de Figma si se afinan los headlines.