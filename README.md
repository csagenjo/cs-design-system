...
> **Estado (08/09/2026, mismo día):** Carol encontró y corrigió un bug real en su propia demo de Figma (gap de 114px en vez de 0 entre items apilados) y cambió el trigger de borde-solo-arriba a borde-arriba-Y-abajo, lo que elimina la necesidad del prop `isLast` por completo — cada item cierra su propio borde inferior siempre, sin depender de su posición en la lista. Bug real encontrado en código de paso: el contenido no hacía fill horizontal (texto plano como children directo de un flex-column se envuelve en una caja anónima sin `width` real) — corregido con un wrapper interno explícito. Nace `AccordionGroup` (organismo) — Carol preguntó si no debíamos tener uno, igual que Table; el propio bug del gap es el argumento: un organismo con `gap: 0` fijo hace ese error estructuralmente imposible. `multiple` decide si se permite más de un item abierto a la vez.
