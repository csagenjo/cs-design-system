...
| `Accordion` | Single expandable/collapsible item · `title`+`children`+`expanded`+`onToggle` · `isLast` decides whether the closing bottom border paints (top border always paints — natural separator between stacked items, same treatment as `lastRow` on CellData/CellHeader) · **2 real IP leaks found and fixed in Figma**: the title text's literal rendered font was a real company's real typeface name despite a correctly-bound `fontFamily/default` Variable, and the top border color was bound to an unresolvable external-library variable ID · **tokens genuinely simplified, not just re-bound** (Carol's explicit ask): 6 fragmented border tokens → 2, title+body text color consolidated into one `fg/text/generic` while icon stays separate as `fg/icon/generic` (project convention: `bg`/`fg`/`border` as parent groups, `fg` splits into `text`/`icon`) | ✅ v1 |
