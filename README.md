...
| `Accordion` | Single expandable/collapsible item · `title`+`children`+`expanded`+`onToggle` · `isLast` decides whether the closing bottom border paints (top border always paints — natural separator between stacked items, same treatment as `lastRow` on CellData/CellHeader) · **2 real IP leaks found and fixed in Figma**: the title text's literal rendered font was a real company's real typeface name despite a correctly-bound `fontFamily/default` Variable, and the top border color was bound to an unresolvable external-library variable ID | ✅ v1 |
